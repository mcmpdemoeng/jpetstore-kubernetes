...
def get_order_number_details(tenant_system_user_name, tenant_system_user_api_key, order_number, tenant_api_url):
    ...
    response, _, _ = common_utils.make_web_request(url=ENDPOINT, headers=headers)
...
def get_petstore_infrasturcture_details(tenant_system_user_name, tenant_system_user_api_key, order_number, tenant_api_url ):
    try:
        print("Gathering petstore infrastructure details...")
        ...
        raise Exception(
            f"Unable to read order '{order_number}' in tenant api url: '{tenant_api_url}'\nError: {err}"
            )
